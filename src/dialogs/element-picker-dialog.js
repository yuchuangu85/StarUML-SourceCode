...
const dialogTemplate = fs.readFileSync(
  path.join(__dirname, "../static/html-contents/element-picker-dialog.html"),
  "utf8",
);
...
class ElementPickerDialog {
  ...
  _setupTree() {
    ...
    this.$tree.on("dblclick", "li.k-item", function (e) {
      if (self.dialog) {
          ...
          this.$treeWrapper.animate(
            {
              scrollTop:
                node.offset().top -
                this.$treeWrapper.offset().top +
                this.$treeWrapper.scrollTop(),
            },
            500,
          );
        }
  ...
  showDialog(title, selected, type) {
    ...
    this.dialog = app.dialogs.showModalDialogUsingTemplate(
      Mustache.render(dialogTemplate, context),
      true,
      ($dlg) => {
        if (
          $dlg.data("buttonId") === "ok" &&
          type !== null &&
          _.isObject(this._selectedElement) &&
          !(this._selectedElement instanceof type)
        ) {
    ...
    var $dlg = this.dialog.getElement();
