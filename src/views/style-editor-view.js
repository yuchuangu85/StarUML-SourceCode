...
class StyleEditorView extends EventEmitter {
  ...
  _setupStyleWidgets() {
    ...
    this.$stereotypeDisplaySelect = this.$view.find(
      ".stereotype-display select",
    );
    ...
    this.$suppressAttributesCheckbox = $(
      "input.suppress-attributes",
      this.$view,
    );
    this.$suppressOperationsButton = $("label.suppress-operations", this.$view);
    this.$suppressOperationsCheckbox = $(
      "input.suppress-operations",
      this.$view,
    );
    this.$suppressReceptionsButton = $("label.suppress-receptions", this.$view);
    this.$suppressReceptionsCheckbox = $(
      "input.suppress-receptions",
      this.$view,
    );
    ...
    self.$fillColor.click(function (e) {
      e.preventDefault();
      const c = self._getFillColor();
      app.dialogs.showColorDialog(c).then(function ({ buttonId, returnValue }) {
        if (buttonId === "ok") {
          self._setFillColor(returnValue);
          self.emit(
            "styleChanged",
            self._views,
            "fillColor",
    ...
    self.$lineColor.click(function (e) {
      e.preventDefault();
      const c = self._getLineColor();
      app.dialogs.showColorDialog(c).then(function ({ buttonId, returnValue }) {
        if (buttonId === "ok") {
          self._setLineColor(returnValue);
          self.emit(
            "styleChanged",
            self._views,
            "lineColor",
    ...
    self.$fontColor.click(function (e) {
      ...
      app.dialogs.showColorDialog(c).then(function ({ buttonId, returnValue }) {
        if (buttonId === "ok") {
          self._setFontColor(returnValue);
          self.emit(
            "styleChanged",
            self._views,
            "fontColor",
    ...
    self.$autoResizeButton.click(function () {
      self.$autoResizeCheckbox.prop(
        "checked",
        !self.$autoResizeCheckbox.is(":checked"),
      );
      self.emit(
        "styleChanged",
        self._views,
    ...
    self.$showShadowButton.click(function () {
      self.$showShadowCheckbox.prop(
        "checked",
        !self.$showShadowCheckbox.is(":checked"),
      );
      self.emit(
        "styleChanged",
        self._views,
    ...
    self.$wordWrapButton.click(function () {
      self.$wordWrapCheckbox.prop(
        "checked",
        !self.$wordWrapCheckbox.is(":checked"),
      );
      self.emit(
        "styleChanged",
        self._views,
    ...
    self.$stereotypeDisplaySelect.change(function () {
      self.emit(
        ...
        self._views,
        "stereotypeDisplay",
        self.$stereotypeDisplaySelect.val(),
      );
    });

    self.$showVisibilityButton.click(function () {
      self.$showVisibilityCheckbox.prop(
        "checked",
        !self.$showVisibilityCheckbox.is(":checked"),
      );
      self.emit(
        "styleChanged",
        self._views,
    ...
    self.$showNamespaceButton.click(function () {
      self.$showNamespaceCheckbox.prop(
        "checked",
        !self.$showNamespaceCheckbox.is(":checked"),
      );
      self.emit(
        "styleChanged",
        self._views,
    ...
    self.$showPropertyButton.click(function () {
      self.$showPropertyCheckbox.prop(
        "checked",
        !self.$showPropertyCheckbox.is(":checked"),
      );
      self.emit(
        "styleChanged",
        self._views,
    ...
    self.$showTypeButton.click(function () {
      self.$showTypeCheckbox.prop(
        "checked",
        !self.$showTypeCheckbox.is(":checked"),
      );
      self.emit(
        "styleChanged",
        self._views,
    ...
    self.$suppressAttributesButton.click(function () {
      self.$suppressAttributesCheckbox.prop(
        "checked",
        !self.$suppressAttributesCheckbox.is(":checked"),
      );
      self.emit(
        "styleChanged",
        self._views,
    ...
    self.$suppressOperationsButton.click(function () {
      self.$suppressOperationsCheckbox.prop(
        "checked",
        !self.$suppressOperationsCheckbox.is(":checked"),
      );
      self.emit(
        "styleChanged",
        self._views,
    ...
    self.$suppressReceptionsButton.click(function () {
      self.$suppressReceptionsCheckbox.prop(
        "checked",
        !self.$suppressReceptionsCheckbox.is(":checked"),
      );
      self.emit(
        "styleChanged",
        self._views,
    ...
    self.$suppressLiteralsButton.click(function () {
      self.$suppressLiteralsCheckbox.prop(
        "checked",
        !self.$suppressLiteralsCheckbox.is(":checked"),
      );
      self.emit(
        "styleChanged",
        self._views,
    ...
    self.$suppressColumnsButton.click(function () {
      self.$suppressColumnsCheckbox.prop(
        "checked",
        !self.$suppressColumnsCheckbox.is(":checked"),
      );
      self.emit(
        "styleChanged",
        self._views,
  ...
  _getFillColor() {
    return this.$fillColor.data("color");
  }

  _setFillColor(color) {
    const elem = this.$fillColor.get(0);
    if (color) {
      this.$fillColor.data("color", color);
  ...
  _getLineColor() {
    return this.$lineColor.data("color");
  }

  _setLineColor(color) {
    const elem = this.$lineColor.get(0);
    if (color) {
      this.$lineColor.data("color", color);
      elem.style.backgroundColor = color;
    } else {
      this.$lineColor.data("color", "");
      elem.style.backgroundColor = "#FFFFFF00";
    }
  }

  _getFontColor() {
    return this.$fontColor.data("color");
  }

  _setFontColor(color) {
    const elem = this.$fontColor.get(0);
    if (color) {
      this.$fontColor.data("color", color);
      elem.style.backgroundColor = color;
    } else {
  ...
  htmlReady() {
    var strings = {
      defaultFonts: Object.keys(app.fontManager.fonts)
        .sort()
        .map((f) => ({ text: f, value: f })),
    };
    const viewTemplate = fs.readFileSync(
      path.join(__dirname, "../static/html-contents/style-editor-view.html"),
      "utf8",
    );
  ...
  appReady() {
    ...
    var defaultFonts = Object.keys(app.fontManager.fonts)
      .sort()
      .map((f) => ({ text: f, value: f }));
