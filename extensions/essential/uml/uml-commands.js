...
const grammarName = fs.readFileSync(
  path.join(__dirname, "/grammars/uml-name.pegjs"),
  "utf8",
);
const grammarAttribute = fs.readFileSync(
  path.join(__dirname, "/grammars/uml-attribute.pegjs"),
  "utf8",
);
const grammarOperation = fs.readFileSync(
  path.join(__dirname, "/grammars/uml-operation.pegjs"),
  "utf8",
);
const grammarTemplateParameter = fs.readFileSync(
  path.join(__dirname, "/grammars/uml-templateparameter.pegjs"),
  "utf8",
);
const grammarLifeline = fs.readFileSync(
  path.join(__dirname, "/grammars/uml-lifeline.pegjs"),
  "utf8",
);
const grammarObject = fs.readFileSync(
  path.join(__dirname, "/grammars/uml-object.pegjs"),
  "utf8",
);
const grammarSlot = fs.readFileSync(
  path.join(__dirname, "/grammars/uml-slot.pegjs"),
  "utf8",
);
const grammarMessage = fs.readFileSync(
  path.join(__dirname, "/grammars/uml-message.pegjs"),
  "utf8",
);
const grammarTransition = fs.readFileSync(
  path.join(__dirname, "/grammars/uml-transition.pegjs"),
  "utf8",
);
...
function handleApplyProfile(fullPath, profileName) {
  if (profileName) {
    ...
    if (
      prj &&
      prj.ownedElements.find((e) => e.name === profileName) instanceof
        type.UMLProfile
    ) {
...
function handleApplyUMLStandardProfile() {
  ...
  if (handleApplyProfile(fullPath, "UMLStandardProfile")) {
    app.toast.info(
      "UMLStandardProfile is successfully applied. Look at under Project.",
    );
  }
}
...
function handleEntryActivity() {
  app.dialogs
    .showSelectRadioDialog("Select one to create as an entry Activity", [
    ...
    .then(function ({ buttonId, returnValue }) {
      if (buttonId === "ok") {
        app.factory.createModel({
          id: returnValue,
          parent: app.selections.getSelected(),
...
function handleDoActivity() {
  app.dialogs
    .showSelectRadioDialog("Select one to create as an do Activity", [
      { text: "OpaqueBehavior", value: "UMLOpaqueBehavior", checked: true },
      { text: "Activity", value: "UMLActivity" },
      { text: "StateMachine", value: "UMLStateMachine" },
      { text: "Interaction", value: "UMLInteraction" },
...
function handleExitActivity() {
  app.dialogs
    .showSelectRadioDialog("Select one to create as an exit Activity", [
      { text: "OpaqueBehavior", value: "UMLOpaqueBehavior", checked: true },
      { text: "Activity", value: "UMLActivity" },
      { text: "StateMachine", value: "UMLStateMachine" },
      { text: "Interaction", value: "UMLInteraction" },
...
function handleEffect() {
  app.dialogs
    .showSelectRadioDialog("Select one to create as an effect Behavior", [
      { text: "OpaqueBehavior", value: "UMLOpaqueBehavior", checked: true },
      { text: "Activity", value: "UMLActivity" },
      { text: "StateMachine", value: "UMLStateMachine" },
      { text: "Interaction", value: "UMLInteraction" },
...
function handleConstraint() {
  ...
  if (parent instanceof type.UMLOperation) {
    app.dialogs
      .showSelectRadioDialog("Select kind of Constraint of Operation", [
    ...
    app.dialogs
      .showSelectRadioDialog("Select kind of Constraint of Behavior", [
        { text: "Typical Constraint", value: "ownedElements", checked: true },
        { text: "Precondition", value: "preconditions" },
        { text: "Postcondition", value: "postconditions" },
      ])
    ...
    app.dialogs
      .showSelectRadioDialog("Select kind of Constraint of Action", [
        { text: "Typical Constraint", value: "ownedElements", checked: true },
...
function handleFrame(options) {
  ...
  app.elementPickerDialog
    .showDialog(
      "Select an element to be represented as Frame",
      null,
      type.Model,
    )
    .then(function ({ buttonId, returnValue }) {
...
function handleNameExpression(options) {
  try {
    ...
    if (ast.stereotype && ast.stereotype.length > 0) {
      let match = app.repository.lookupAndFind(
        options.model,
        ast.stereotype,
        type.UMLStereotype,
      );
...
function handleAttributeExpression(options) {
  try {
    ...
    // stereotype
    if (ast.stereotype && ast.stereotype.length > 0) {
      match = app.repository.lookupAndFind(
        options.model,
        ast.stereotype,
        type.UMLStereotype,
      );
...
function handleTemplateParameterExpression(options) {
  try {
    ...
    // stereotype
    if (ast.stereotype && ast.stereotype.length > 0) {
      var match = app.repository.lookupAndFind(
        options.model,
        ast.stereotype,
        type.UMLStereotype,
      );
...
function handleAddNote(options) {
  ...
  let area =
    options.view instanceof type.UMLGeneralEdgeView
      ? view.nameLabel.getBoundingBox()
      : view.getBoundingBox();
  ...
  if (
    options2.tailView instanceof type.EdgeLabelView ||
    options2.tailView instanceof type.NodeLabelView
  ) {
    options2.tailView = options2.tailView._parent;
  }
  if (
    options2.headView instanceof type.EdgeLabelView ||
    options2.headView instanceof type.NodeLabelView
  ) {
...
function handleAddConstraint(options) {
  try {
    let diagram = app.diagrams.getEditor().diagram;
    let nodes = app.repository.getConnectedNodeViews(
      options.view,
      type.UMLConstraintLinkView,
    );
    let area =
      options.view instanceof type.UMLGeneralEdgeView
        ? options.view.nameLabel.getBoundingBox()
        : options.view.getBoundingBox();
...
function handleAddCompartmentItem(options) {
  options["parent-model"] = options["parent-model"]
    ? _.get(options, options["parent-model"])
    : options.model._parent;
  options["parent-view"] = options["parent-view"]
    ? _.get(options, options["parent-view"])
    : options.view._parent;
  ...
  item.name = type.Element.getNewName(
    options["parent-model"][options.field],
    options["name-prefix"],
  );
...
function handleOpenUpCompartmentItem(options) {
  ...
  if (
    !(options.view._parent instanceof type.UMLCompartmentView) &&
    options.view.nameCompartment
  ) {
    compartmentItem = options.view.nameCompartment.nameLabel;
  }
  let compartmentOwner = compartmentItem._parent._parent;
  let compartments = compartmentOwner.getAllCompartments
    ? compartmentOwner.getAllCompartments()
    : [compartmentItem._parent];
  ...
  if (
    prevIdx < 0 ||
    currentCompartment instanceof type.UMLNameCompartmentView
  ) {
    ...
    if (compartmentIdx > 0) {
      ...
      if (prevCompartment) {
        let targetIdx = _getPrevVisible(
          prevCompartment.subViews,
          prevCompartment.subViews.length - 1,
        );
...
function handleAddSuperClass(options) {
  const diagram = app.diagrams.getCurrentDiagram();
  const diagramOwner = diagram._parent;
  const nodes = app.quickedits.getHeadNodes(
    options.view,
    type.UMLGeneralizationView,
  );
  const options1 = Object.assign(
...
function handleAddRealizingClass(options) {
  ...
  const nodes = app.quickedits.getTailNodes(
    view,
    type.UMLInterfaceRealizationView,
  );
  const options1 = Object.assign(
    {
      id: "UMLClass",
...
function handleAddProvidedInterface(options) {
  ...
  const nodeView = app.factory.createModelAndView(options1);
  if (
    view.model instanceof type.UMLPort &&
    !(view.model.type instanceof type.UMLClassifier)
  ) {
...
function handleAddAssociatedClass(options) {
  ...
  const diagram = app.diagrams.getCurrentDiagram();
  const diagramOwner = diagram._parent;
  const nodes = app.quickedits.getBothNodes(
    view,
    type.UMLAssociationView,
    function (e) {
      return (
        e.tail === view && e.model.end1.aggregation === UMLAttribute.AK_NONE
      );
    },
  );
  const options1 = Object.assign(
...
function handleAddAggregatedClass(options) {
  ...
  const nodes = app.quickedits.getBothNodes(
    ...
    function (e) {
      return (
        e.tail === view && e.model.end1.aggregation === UMLAttribute.AK_SHARED
      );
    },
  );
  const options1 = Object.assign(
...
function handleAddCompositedClass(options) {
  ...
  const nodes = app.quickedits.getBothNodes(
    ...
    function (e) {
      return (
        e.tail === view &&
        e.model.end1.aggregation === UMLAttribute.AK_COMPOSITE
      );
    },
  );
  const options1 = Object.assign(
...
function handleAddPart(options) {
  const view = options.view;
  const diagram = app.diagrams.getCurrentDiagram();
  const nodes = _.filter(view.containedViews, function (e) {
    return e instanceof type.UMLPartView;
  });
  const options1 = {
    id: "UMLPart",
    diagram: diagram,
  ...
  if (nodes.length > 0) {
    var _rightMost = _.max(nodes, function (e) {
      return e.getRight();
    });
...
function handleSelectOwnerAttribute(options) {
  ...
  app.elementListPickerDialog
    .showDialog("Select Owner Attribute", attrs)
    .then(function ({ buttonId, returnValue }) {
      if (buttonId === "ok") {
...
function handleSelectSignal(options) {
  const view = options.view;
  app.elementPickerDialog
    .showDialog("Select Signal", view.model.signature, type.UMLSignal)
    .then(function ({ buttonId, returnValue }) {
      if (buttonId === "ok") {
...
function handleSelectType(options) {
  ...
  if (view.model instanceof type.UMLAttribute) {
    app.elementPickerDialog
      .showDialog("Select Type", view.model.type, type.UMLClassifier)
      .then(function ({ buttonId, returnValue }) {
        if (buttonId === "ok") {
...
function handleCreateType(options) {
  ...
  if (view.model instanceof type.UMLAttribute) {
    ...
    if (parent) {
      app.dialogs
        .showInputDialog("Enter a type name to create")
        .then(function ({ buttonId, returnValue }) {
...
function handleSelectTypeForLifeline(options) {
  ...
  if (view.model.represent instanceof type.UMLAttribute) {
    app.elementPickerDialog
      .showDialog("Select Type", view.model.represent.type, type.UMLClassifier)
      .then(function ({ buttonId, returnValue }) {
        if (buttonId === "ok") {
    ...
    app.dialogs.showAlertDialog(
      "Lifeline should represent a role. (connect 'represent' field to an Attribute of the container Collaboration)",
    );
  }
}
...
function newSeqLifelinePosition(diagram) {
  var lifelines = _.filter(diagram.ownedViews, function (e) {
    return e instanceof type.UMLSeqLifelineView;
  });
  var rightMost = _.max(lifelines, function (e) {
    return e.getRight();
  });
  var options = {
...
function newSeqMessagePosition(fromLifeline, toLifeline, diagram, interval) {
  ...
  var options2 = {
    ...
    x2: toLifeline.left,
    y1:
      fromLifeline instanceof type.UMLSeqLifelineView
        ? fromLifeline.linePart.top + 20
        : fromLifeline.top,
    y2:
      fromLifeline instanceof type.UMLSeqLifelineView
        ? fromLifeline.linePart.top + 20
  ...
  if (messages.length > 0) {
    var bottomMost = _.max(messages, function (e) {
      return e.points.points[0].y;
    });
...
function handleSelectOperation(options) {
  const view = options.view;
  if (
    view.model.target.represent instanceof type.UMLAttribute &&
    view.model.target.represent.type instanceof type.UMLClassifier
  ) {
    var ops = _.union(
      view.model.target.represent.type.getInheritedOperations(true),
      view.model.target.represent.type.operations,
    );
    app.elementListPickerDialog
      .showDialog("Select Operation", ops)
      .then(function ({ buttonId, returnValue }) {
        if (buttonId === "ok") {
          app.engine.setProperty(view.model, "signature", returnValue);
        }
...
function handleCreateOperation(options) {
  ...
  if (
    ...
    app.dialogs
      .showInputDialog("Enter an operation name to create")
      .then(function ({ buttonId, returnValue }) {
        if (buttonId === "ok" && returnValue.length > 0) {
          const builder = app.repository.getOperationBuilder();
...
function handleCreateSignal(options) {
  ...
  if (parent) {
    app.dialogs
      .showInputDialog("Enter a signal name to create")
      .then(function ({ buttonId, returnValue }) {
        if (buttonId === "ok" && returnValue.length > 0) {
          const builder = app.repository.getOperationBuilder();
...
function handleAddReplyMessage(options) {
  ...
  if (diagram instanceof type.UMLSequenceDiagram) {
    ...
    if (
      view.head instanceof type.UMLLinePartView &&
      view.tail instanceof type.UMLLinePartView
    ) {
      const options2 = {
        id: "UMLMessage",
        diagram: diagram,
    ...
    const options2 = {
      ...
      direction:
        view.model.source === view.hostEdge.tail.model ? "reverse" : "forward",
  ...
  app.dialogs
    ...
    .then(function ({ buttonId, returnValue }) {
      if (buttonId === "ok") {
        ...
        switch (returnValue) {
          case "1":
            return handleAddCompartmentItem(
              Object.assign({ type: "UMLOpaqueBehavior" }, options1, options),
            );
          case "2":
            return handleAddCompartmentItem(
              Object.assign({ type: "UMLActivity" }, options1, options),
            );
          case "3":
            return handleAddCompartmentItem(
              Object.assign({ type: "UMLStateMachine" }, options1, options),
            );
          case "4":
            return handleAddCompartmentItem(
              Object.assign({ type: "UMLInteraction" }, options1, options),
            );
        }
      }
    });
...
function handleAddDoActivity(options) {
  app.dialogs
    .showSelectRadioDialog("Select one to create as a do Activity", [
      { text: "OpaqueBehavior", value: "1", checked: true },
      { text: "Activity", value: "2" },
      { text: "StateMachine", value: "3" },
      { text: "Interaction", value: "4" },
...
function handleAddOutgoingControlFlow(options) {
  ...
  const diagramOwner = diagram._parent;
  if (
    view.model instanceof type.UMLObjectNode &&
    view.model.isControlType !== true
  ) {
...
function updateMenus() {
  ...
  let isPackage =
    selected instanceof type.UMLPackage ||
    selected instanceof type.UMLSubsystem;
  ...
  function hasNoInteractionDiagram() {
    return (
      !selected.ownedElements.some(
        (e) => e instanceof type.UMLSequenceDiagram,
      ) &&
      !selected.ownedElements.some(
        (e) => e instanceof type.UMLCommunicationDiagram,
      ) &&
      !selected.ownedElements.some((e) => e instanceof type.UMLTimingDiagram) &&
      !selected.ownedElements.some(
        (e) => e instanceof type.UMLInteractionOverviewDiagram,
      )
    );
  }
  ...
  let visibleStates = {
    ...
    "uml.sequence-diagram":
      ...
      isClassifier ||
      isOperation ||
      (isInteraction && hasNoInteractionDiagram()),
    ...
    "uml.information-flow-diagram":
      isNone || isProject || isPackage || isClassifier,
    ...
    "uml.parameter": isOperation || isActivity,
  ...
  let enabledStates = {
    ...
    "format.show-diagram-name": views.length > 0,
  ...
  let checkedStates = {
    ...
    "format.stereotype.decoration":
      stereotypeDisplay === UMLGeneralNodeView.SD_DECORATION,
    "format.stereotype.decoration-label":
      stereotypeDisplay === UMLGeneralNodeView.SD_DECORATION_LABEL,
    "format.stereotype.icon": stereotypeDisplay === UMLGeneralNodeView.SD_ICON,
    "format.stereotype.icon-label":
      stereotypeDisplay === UMLGeneralNodeView.SD_ICON_LABEL,
    ...
    "format.show-operation-signature": Element.mergeProps(
      views,
      "showOperationSignature",
    ),
    "format.show-diagram-name": Element.mergeProps(views, "showDiagramName"),
    "format.suppress-attributes": Element.mergeProps(
      views,
      "suppressAttributes",
    ),
    "format.suppress-operations": Element.mergeProps(
      views,
      "suppressOperations",
    ),
    "format.suppress-receptions": Element.mergeProps(
      views,
      "suppressReceptions",
    ),
...
app.commands.register(
  ...
  handleApplyUMLStandardProfile,
  "UML: Apply UML Standard Profile",
);
...
app.commands.register(
  ...
  handleAttributeExpression,
);
app.commands.register(
  "uml:set-operation-expression",
  handleOperationExpression,
);
app.commands.register(
  "uml:set-template-parameter-expression",
  handleTemplateParameterExpression,
);
app.commands.register(
  "uml:set-constraint-specification",
  handleConstraintSpecification,
);
...
app.commands.register(
  ...
  handleDeleteCompartmentItem,
);
app.commands.register(
  "uml:move-up-compartment-item",
  handleMoveUpCompartmentItem,
);
app.commands.register(
  "uml:move-down-compartment-item",
  handleMoveDownCompartmentItem,
);
app.commands.register(
  "uml:open-up-compartment-item",
  handleOpenUpCompartmentItem,
);
app.commands.register(
  "uml:open-down-compartment-item",
  handleOpenDownCompartmentItem,
);
app.commands.register(
  "uml:add-template-parameter-substitution",
  handleAddTemplateParameterSubstitution,
);
...
app.commands.register(
  ...
  handleAddOutgoingControlFlow,
);
app.commands.register(
  "uml:add-incoming-control-flow",
  handleAddIncomingControlFlow,
);
app.commands.register(
  "uml:add-outgoing-object-flow",
  handleAddOutgoingObjectFlow,
);
app.commands.register(
  "uml:add-incoming-object-flow",
  handleAddIncomingObjectFlow,
);
...
app.commands.register(
  ...
  _.partial(handleStereotypeDisplay, UMLGeneralNodeView.SD_NONE),
  "UML: Stereotype Display > None",
);
app.commands.register(
  "format:stereotype-label",
  _.partial(handleStereotypeDisplay, UMLGeneralNodeView.SD_LABEL),
  "UML: Stereotype Display > Label",
);
app.commands.register(
  "format:stereotype-decoration",
  _.partial(handleStereotypeDisplay, UMLGeneralNodeView.SD_DECORATION),
  "UML: Stereotype Display > Decoration",
);
app.commands.register(
  "format:stereotype-decoration-label",
  _.partial(handleStereotypeDisplay, UMLGeneralNodeView.SD_DECORATION_LABEL),
  "UML: Stereotype Display > Decoration with Label",
);
app.commands.register(
  "format:stereotype-icon",
  _.partial(handleStereotypeDisplay, UMLGeneralNodeView.SD_ICON),
  "UML: Stereotype Display > Icon",
);
app.commands.register(
  "format:stereotype-icon-label",
  _.partial(handleStereotypeDisplay, UMLGeneralNodeView.SD_ICON_LABEL),
  "UML: Stereotype Display > Icon with Label",
);
app.commands.register(
  "format:word-wrap",
  _.partial(handleToggleProperty, "wordWrap"),
  "Word Wrap",
);
app.commands.register(
  "format:show-visibility",
  _.partial(handleToggleProperty, "showVisibility"),
  "UML: Show Visibility",
);
app.commands.register(
  "format:show-namespace",
  _.partial(handleToggleProperty, "showNamespace"),
  "UML: Show Namespace",
);
app.commands.register(
  "format:show-property",
  _.partial(handleToggleProperty, "showProperty"),
  "UML: Show Property",
);
app.commands.register(
  "format:show-type",
  _.partial(handleToggleProperty, "showType"),
  "UML: Show Type",
);
app.commands.register(
  "format:show-multiplicity",
  _.partial(handleToggleProperty, "showMultiplicity"),
  "UML: Show Multiplicity",
);
app.commands.register(
  "format:show-operation-signature",
  _.partial(handleToggleProperty, "showOperationSignature"),
  "UML: Show Operation Signature",
);
app.commands.register(
  "format:show-diagram-name",
  _.partial(handleToggleProperty, "showDiagramName"),
  "UML: Show Diagram Name",
);
app.commands.register(
  "format:suppress-attributes",
  _.partial(handleToggleProperty, "suppressAttributes"),
  "UML: Suppress Attributes",
);
app.commands.register(
  "format:suppress-operations",
  _.partial(handleToggleProperty, "suppressOperations"),
  "UML: Suppress Operations",
);
app.commands.register(
  "format:suppress-receptions",
  _.partial(handleToggleProperty, "suppressReceptions"),
  "UML: Suppress Receptions",
);
app.commands.register(
  "format:suppress-literals",
  _.partial(handleToggleProperty, "suppressLiterals"),
  "UML: Suppress Literals",
);
