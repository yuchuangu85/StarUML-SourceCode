...
function portFn(parent, diagram, options) {
  var model, view;
  app.factory.assert(
    parent instanceof type.SysMLBlock ||
      parent instanceof type.SysMLProperty ||
      (parent instanceof type.SysMLInternalBlockDiagram &&
        parent._parent instanceof type.SysMLBlock),
...
function connectorFn(parent, diagram, options) {
  app.factory.assert(
    (options.tailModel instanceof type.SysMLProperty ||
      options.tailModel instanceof type.SysMLPort) &&
      (options.tailModel instanceof type.SysMLProperty ||
        options.tailModel instanceof type.SysMLPort),
...
function parameterFn(parent, diagram, options) {
  app.factory.assert(
    parent instanceof type.SysMLProperty ||
      (parent instanceof type.SysMLInternalBlockDiagram &&
        parent._parent instanceof type.SysMLBlock) ||
      (parent instanceof type.SysMLParametricDiagram &&
        parent._parent instanceof type.SysMLBlock),
    Mustache.render(
      "Constraint Parameter should be placed in a block or a part",
    ),
  );
  if (parent instanceof type.SysMLProperty) {
    app.factory.assert(
  ...
  return app.factory.defaultModelAndViewFn(
    parent,
    diagram,
...
function viewForBlockDefinitionDiagramFn(model, diagram, options) {
  ...
  if (model instanceof type.SysMLPort) {
    if (
      containerView instanceof type.SysMLBlockView &&
      containerView.model === model._parent
    ) {
      return app.factory.createViewAndRelationships(
        editor,
        x,
        y,
        model,
      ...
      app.dialogs.showAlertDialog(
        "Port should be dropped on it's parent Block",
      );
    }
  } else {
    ...
    if (model._parent === parent) {
      if (parent.constraints && parent.constraints.includes(model)) {
        return app.factory.createViewAndRelationships(
          ...
          model,
          containerView,
          type.SysMLConstraintPropertyView,
        );
      } else {
        return app.factory.createViewAndRelationships(
          editor,
      ...
      model._parent instanceof type.SysMLConstraintBlock &&
      model._parent.parameters &&
      model._parent.parameters.includes(model)
    ) {
      if (
        containerView instanceof type.SysMLConstraintPropertyView &&
        containerView.model.type === model._parent
      ) {
        return app.factory.createViewAndRelationships(
          editor,
          x,
          ...
          model,
          containerView,
          type.SysMLConstraintParameterView,
        );
      } else {
        app.dialogs.showAlertDialog(
          "Constraint Parameter should be dropped on a it's parent Constraint Parameter",
        );
      }
    } else {
      app.dialogs.showAlertDialog(
        "Only part in the block context can be dropped",
      );
    }
  } else if (model instanceof type.SysMLPort) {
    if (
      containerView instanceof type.SysMLPartView &&
      containerView.model.type === model._parent
    ) {
      return app.factory.createViewAndRelationships(
        editor,
      ...
      app.dialogs.showAlertDialog(
        "Port should be dropped on a part whose type is the port's parent Block",
      );
    }
...
app.factory.registerModelAndViewFn("SysMLConform", _directedRelationshipFn, {
  precondition: modelElementLinkPrecondition,
});
app.factory.registerModelAndViewFn("SysMLExpose", _directedRelationshipFn, {
  precondition: modelElementLinkPrecondition,
});
app.factory.registerModelAndViewFn("SysMLRequirement", _modelAndViewFn);
app.factory.registerModelAndViewFn("SysMLCopy", _directedRelationshipFn, {
  precondition: modelElementLinkPrecondition,
});
app.factory.registerModelAndViewFn("SysMLDeriveReqt", _directedRelationshipFn, {
  precondition: modelElementLinkPrecondition,
});
app.factory.registerModelAndViewFn("SysMLVerify", _directedRelationshipFn, {
  precondition: modelElementLinkPrecondition,
});
app.factory.registerModelAndViewFn("SysMLSatisfy", _directedRelationshipFn, {
  precondition: modelElementLinkPrecondition,
});
app.factory.registerModelAndViewFn("SysMLRefine", _directedRelationshipFn, {
  precondition: modelElementLinkPrecondition,
});
...
app.factory.registerViewOfFn(
  "SysMLRequirementDiagram",
  viewForGeneralDiagramFn,
);
app.factory.registerViewOfFn(
  "SysMLBlockDefinitionDiagram",
  viewForBlockDefinitionDiagramFn,
);
app.factory.registerViewOfFn(
  "SysMLInternalBlockDiagram",
  viewForInternalBlockDiagramFn,
);
app.factory.registerViewOfFn(
